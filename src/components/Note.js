...
class Note extends React.Component{
    ...
    componentDidMount(){
      ...
      axios.get(`http://localhost:9000/index/${id}/see_all_noteslabels`)
      .then(response =>{
        this.setState({
    ...
    render(){
    const allNL = this.state.allNotesLabels.map(nl => { return( <li key={nl.id}>{nl.labels}</li>)})
      return(
        <>
